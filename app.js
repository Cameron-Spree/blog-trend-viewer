...
function setupFileInput() {
    ...
    console.log('[INIT] File input listener attached');
    
    input.addEventListener('change', function(e) {
        console.log('[FILE] Change event fired, files:', e.target.files.length);
...
function handleFile(file) {
    console.log('[HANDLE] Processing file:', file.name);
...
function processCSVData(data) {
    ...
    console.log('[CSV] Detected columns - URL:', urlKey, 'Clicks:', clicksKey, 'Impressions:', impressionsKey);

    if (!urlKey || !clicksKey) {
        alert("Could not identify URL or Clicks columns in the CSV.\n\nDetected columns: " + keys.join(', '));
    ...
    const blogs = data.filter(row => {
        ...
        ctr: row[keys.find(k => k.toLowerCase().includes('ctr'))] || 0,
        position: row[keys.find(k => k.toLowerCase().includes('position'))] || 0,
    }));

    console.log('[CSV] Blog URLs found:', blogs.length);

    if (blogs.length === 0) {
        alert("No URLs containing '/blog/' found in the CSV.\n\nFirst URL in CSV: " + (data[0][urlKey] || 'N/A'));
    ...
    State.enhancedData = [];
    State.batchQueue = [...blogs];
    State.isProcessing = false;
    
    console.log('[CSV] Queued', State.batchQueue.length, 'URLs for crawling');
    startCrawling();
}
...
async function startCrawling() {
    if (State.isProcessing) {
        console.warn('[CRAWL] Already processing, skipping');
        return;
    }
    ...
    console.log('[CRAWL] Starting batch processing of', State.totalToProcess, 'URLs');
    ...
    while (State.batchQueue.length > 0) {
        ...
        console.log('[CRAWL] Batch', batchNumber, '- processing', currentBatch.length, 'URLs');
        ...
        await Promise.all(currentBatch.map(async (item) => {
            try {
                ...
                console.warn('[CRAWL] Failed:', item.url, err.message);
                State.enhancedData.push({ ...item, wordCount: 0, headingCount: 0, h1Count: 0, h2Count: 0, h3Count: 0, imageCount: 0, elementsCount: 0, publishDate: null, category: 'Uncategorized' });
            } finally {
                State.completedCount++;
                const perc = (State.completedCount / State.totalToProcess) * 100;
                progressFill.style.width = `${perc}%`;
                progressText.innerText = `${State.completedCount} / ${State.totalToProcess} URLs Processed`;
            }
        }));
...
function showBatchPrompt(batchNum) {
    console.log('[BATCH] Batch', batchNum, 'complete. Total data:', State.enhancedData.length);
...
function refreshDashboardData() {
    console.log('[REFRESH] Refreshing charts with', State.enhancedData.length, 'items');
    analyzeData();
    const activeTab = document.querySelector('.nav-item.active');
    if (activeTab) renderChartsForTab(activeTab.dataset.tab);
...
function parseHTMLForMetrics(htmlString) {
    if (!htmlString) return { wordCount: 0, headingCount: 0, h1Count: 0, h2Count: 0, h3Count: 0, imageCount: 0, elementsCount: 0, publishDate: null, category: 'Uncategorized' };
...
function finishCrawling() {
    ...
    console.log('[DONE] Crawling finished. Total items:', State.enhancedData.length);
    
    // Auto-switch to Date tab
    const dateTab = document.querySelector('[data-tab="date"]');
    if (dateTab) dateTab.click();
    
    // Clear any remaining toast
    const toast = document.getElementById('batch-toast');
    if (toast) toast.remove();
}
...
function buildContentCharts() {
    ...
    createChart('chart-images', {
        type: 'scatter',
        data: { datasets: [{ label: 'Images vs Clicks', data: validData.map(d => ({ x: d.imageCount || 0, y: d.clicks })), backgroundColor: '#ef4444' }] },
        options: {
            responsive: true, maintainAspectRatio: false,
    ...
    validData.forEach(d => {
        ...
        let k = hc <= 5 ? '0-5' : hc <= 10 ? '6-10' : hc <= 15 ? '11-15' : '16+';
        hGroup[k].cl += Number(d.clicks) || 0;
        hGroup[k].ct += 1;
    });
...
function renderChartsForTab(tabName) {
    setTimeout(() => {
        Object.values(State.charts).forEach(chart => {
            if (chart.canvas && chart.canvas.closest(`#tab-${tabName}`)) chart.resize();
        });
    }, 50);
}
